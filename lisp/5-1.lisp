...
(defun at-loc-p (obj loc)
    (eq (cadr (assoc obj *object-locations*)) loc))
...
(defun pickup (object)
    (cond ((member object
                ...
                ;;; tend that values in an alist are changing, while still preserving old values.
...
(defun game-read ()
    (let ((cmd (read-from-string (concatenate 'string "(" (read-line) ")"))))
        (flet ((quote-it (x) (list 'quote x)))
            (cons (car cmd) (mapcar #'quote-it (cdr cmd)))))) ; convert latter part of cmd to symbols
