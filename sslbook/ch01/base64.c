...
int base64_decode( const unsigned char *input, int len, unsigned char *output )
{
  ...
  // 4 * 6 = 24; 3 * 8 = 24
  // Encoding process: 3 bytes -> 4 ascii char
  // Decoding process: 4 ascii char (one byte represents 6 bits) -> 3 bytes
  // (4 * 6 = 24)
  // (24 / 8 = 3)
  assert( !( len & 0x03 ) );
  ...
  do
  {
    for ( i = 0; i <= 3; i++ ) // process 4 ascii characters
	// in: 4 ascii character; out: 3 bytes
    {
...
#ifdef BASE64_MAIN
int main() {
	printf("sizeof unbase64: %d\n", sizeof(unbase64)/sizeof(int));
	return 0;
}
#endif
